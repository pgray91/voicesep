...
import voicesep.separators.neural.chord_level.features
import voicesep.separators.neural.chord_level.neural_network
...
class TestNetwork(unittest.TestCase):
  ...
  def setUp(self):
    ...
    self.features = vs.neural.chord_level.Features(
  ...
  def test_00_1H(self):
    ...
    network = vs.separators.neural.chord_level.neural_network.NeuralNetwork(
      ...
      "sigmoid", None,
      grad.sgd, {"lr" : 0.1}, 
      margin=1, l2_reg=0.001, 
      assign_limit=64, batch_size=2,
      mode="FAST_RUN"
    )

    network.train(
      self.train_scores, self.active_voices, self.features,
      epochs=2, batch_size=2, cprint=1
    )
  ...
  def test_01_2H(self):
    ...
    network = vs.separators.neural.chord_level.neural_network.NeuralNetwork(
      ...
      "sigmoid", None,
      grad.sgd, {"lr" : 0.1}, 
      assign_limit=64, margin=1, l2_reg=0.001, mode="FAST_RUN",
      batch_size=2
    )

    network.train(
      self.train_scores, self.active_voices, self.features,
      epochs=6, batch_size=2, cprint=1
    )
  ...
  def test_04_1H_1I(self):
    ...
    network.train(
      self.train_scores, self.active_voices, self.features,
      epochs=10, batch_size=2, cprint=1
    )
  ...
  def test_05_sizes(self):
    ...
    network = vs.separators.neural.chord_level.neural_network.NeuralNetwork(
      ...
      vs.neural.chord_level.Features.GROUP_PADS,
      (10, 10, 10), "sigmoid", 15,
      (20, 10, 6, 1), (True, True, False),
      "sigmoid", None,
      grad.sgd, {"lr" : 0.1}, 
      assign_limit=64, margin=1, l2_reg=0.001, mode="FAST_RUN", batch_size=2
    )
