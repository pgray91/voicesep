...
class Convolutional:
    ...
    def __init__(self, X_var, input_size, output_size, vector_count, activation):
        ...
        a_var = getattr(activations, activation)(
            T.dot(X_var, self.W_var) + self.b_var
        )
        
        self.y_hat_var = (
            a_var.reshape(
                (a_var.shape[0] // vector_count, vector_count, output_size)
            )
            .argmax(axis=1)
        )
